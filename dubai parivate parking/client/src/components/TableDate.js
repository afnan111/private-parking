...
import {checkinReqDeletedByAdmin} from "../actions/Issue_action"
...
const TableDate = ({book,fetchDataAgaib}) => {
    ...
    dayDiff = days
     ...
     const clearcheckinBookqq =(bookId)=>{
        dispatch(checkinReqDeletedByAdmin(bookId))
        fetchDataAgaib()
     }
    ...
    checkout (
        ...
        <td>
             {/* <i className='fa fa-trash m-1' onClick={()=> console.log("okk")}></i>
              ...
              <button onClick={() => console.log("ok")} className="btn btn-danger ">checkout </button> */}
             <button onClick={() =>  clearcheckinBookqq(book._id)} className="btn btn-danger">Clear</button>
        </td>
